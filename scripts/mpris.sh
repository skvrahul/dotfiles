...
# Credit for this script: https://github.com/snickerton/DotfilesSucculent/blob/master/.config/polybar/mpris.sh
# requires jq to work
...
if [[ $player_status = "true" ]]; then
    echo "%{F#FC3}$icon $metadata"       # Orange when playing
elif [[ $player_status = "false" ]]; then
    echo "%{F#65737E}$icon -paused- "       # Greyed out info when paused
else
